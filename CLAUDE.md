...
- **CSS**: SCSS compiled via `sass-loader` and extracted by `mini-css-extract-plugin` into `bundle.[contenthash].css`, linked render-blocking in `<head>` — never inject it from the deferred JS bundle (`style-loader`): on a slow connection the page would first paint unstyled, the logo at full size. `css-loader`'s `url.filter` deliberately skips URLs starting with `/public/` so the hero backgrounds resolve at runtime instead of being bundled.
- **Fonts**: as in the print documents in `aqualign-docs`: Source Sans 3 (400, 400 italic, 700) for running text, Didact Gothic — the typeface of the logo wordmark — for headings and the header (`--font-body` / `--font-display` in `global.scss`). Self-hosted from `fonts/` as `woff2` subsets (latin, latin-ext) via the `face()` mixin in `src/css/fonts.scss`; no Google Fonts request. Didact Gothic has one weight only, so headings stay `font-weight: normal`. `fonts/DidactGothic-Regular.ttf` is only for `generate-og-image.js` (fontconfig cannot read woff2).
...
**Share image**: `public/og-image.jpg` (1200×630) is built by `scripts/generate-og-image.js` — the hero photo, the logo rasterised from `Logo.svg`, the wordmark (outlined paths read from `partials/_wordmark.ejs`) and the subtitle, composited by Sharp. It is committed and regenerated **manually** via `npm run og-image`, deliberately not wired into the prebuild hooks: the subtitle is rendered through fontconfig, and a CI runner's font set differs from a developer machine, so building it there would silently substitute a fallback for Didact Gothic. `generate-images.js` skips it by name, since a fixed-size social asset needs no responsive variants. Social crawlers do not render SVG, so the logo cannot be used as `og:image` directly.
...
- The mode's body class is set before the first paint by the inline script in `partials/_initial-mode.ejs` (first thing in every `<body>`), which mirrors `site-state.js`'s resolution — the stylesheet blocks rendering but the bundle is deferred, so otherwise the landing page would first paint with both sections visible. Keep the two in sync if the mode logic, hash names or storage key change
- The initial mode is applied without animation: `no-initial-transitions.js` (first in `main.js`) sets `html.no-transitions`, which disables every transition, and lifts it two frames later — otherwise a page opened in Eisbaden mode fades in from the warm defaults
...
- The heading "Aqualign Swim & Ice" is the logo wordmark from the brand lockups in `aqualign-docs/logos/` (`*_C-welle.svg`), inlined as SVG by `partials/_wordmark.ejs` and recoloured by `.wordmark` in `header.scss` (name in `--main-color`, tagline and waves in `--medium-color`) — the lockups themselves are navy for print on white. The text is carried by an `.sr-only` span beside it. The compact header of the content pages uses the horizontal A lockup instead (`partials/_wordmark-horizontal.ejs`, icon beside the wordmark)
- The toggle is a styled CSS checkbox (`#switch`), only present on `index.ejs`. The last mode is kept in `sessionStorage` (`aqualign-mode`): pages without the switch apply its body class, and the landing page without a hash reopens it (hash > stored mode > default warm)
